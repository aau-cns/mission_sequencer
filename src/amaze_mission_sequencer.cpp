  ...
  nh_(nh), pnh_(pnh)
{
    ...
    rosSubscriberExtendedVehicleState_ = nh.subscribe("/mavros/extended_state", 10, &AmazeMissionSequencer::rosExtendedVehicleStateCallback, this);
    rosSubscriberVehiclePose_ = nh.subscribe("/mavros/local_position/pose", 10, &AmazeMissionSequencer::rosPoseCallback, this);
    ...
    rosSubscriberExtendedVehicleState_ = nh_.subscribe("mavros/extended_state", 10, &AmazeMissionSequencer::rosExtendedVehicleStateCallback, this);
    rosSubscriberVehiclePose_ = nh_.subscribe("mavros/local_position/pose", 10, &AmazeMissionSequencer::rosPoseCallback, this);
    rosSubscriberRequest_ = nh_.subscribe("autonomy/request", 10, &AmazeMissionSequencer::rosRequestCallback, this);
    rosSubscriberWayPointFileName_ = pnh_.subscribe("waypoint_filename", 10, &AmazeMissionSequencer::rosWaypointFilenameCallback, this);
...
void AmazeMissionSequencer::rosPoseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    if (!poseValid_ || (currentFollowerState_ == IDLE))
    {
        ...
        tf2::Quaternion q_ENU_BODY(startingVehiclePose_.pose.orientation.x, startingVehiclePose_.pose.orientation.y,startingVehiclePose_.pose.orientation.z, startingVehiclePose_.pose.orientation.w);
        ...
        if(verbose_){ ROS_INFO_STREAM_THROTTLE(dbg_throttle_rate_, "* Initial (PX4/NED) yaw= " << startingYaw*RAD_TO_DEG << "[deg], (OptiTrack/ENU) pos x=" << startingVehiclePose_.pose.position.x << " pos y=" << startingVehiclePose_.pose.position.y << " pos z=" << startingVehiclePose_.pose.position.z); }
...
bool AmazeMissionSequencer::getFilenames()
{
    ...
    else
    {
        ...
        {
            // [TODO] Manage error
            ROS_WARN_STREAM("AmazeMissionSequencer::getFilenames(): failure! Cound not get file paths for mission:" << std::to_string(missionID_));
            return false;
        }
...
void AmazeMissionSequencer::rosRequestCallback(const amaze_mission_sequencer::request::ConstPtr& msg)
{
    ...
    {
        if (int(msg->request) == amaze_mission_sequencer::request::READ && (currentFollowerState_ == IDLE || currentFollowerState_ == PREARM) )
        {
            currentFollowerState_ = IDLE;
        ...
        {
            ROS_INFO_STREAM("WRONG MISSION ID FOR CURRENT STATE: " << StateStr[currentFollowerState_] << "; Local mission ID:" << missionID_ << " msg ID: " << int(msg->id));
    ...
    switch (int(msg->request))
    {
        case amaze_mission_sequencer::request::READ:
            if(verbose_){ ROS_INFO_STREAM("* amaze_mission_sequencer::request::READ..."); }
            ...
            {
                if(verbose_){ ROS_WARN_STREAM("* amaze_mission_sequencer::request::READ - failed! Not in IDLE nor!"); }
                wrongInput = true;
            }
            break;
        ...
        case amaze_mission_sequencer::request::ARM:
            ...
            {
                if(verbose_){ ROS_WARN_STREAM("* amaze_mission_sequencer::request::ARM - failed! Not in PREARM!"); }
                if(verbose_){ ROS_WARN_STREAM("*   Valids: Pose=" << poseValid_ << ", State=" << stateValid_ << ", extState=" << extendedStateValid_); }
                wrongInput = true;
            }
            break;

        case amaze_mission_sequencer::request::HOLD:
            if(verbose_){ ROS_INFO_STREAM("* amaze_mission_sequencer::request::HOLD..."); }
            if (currentFollowerState_ == MISSION)
            {
                ROS_INFO_STREAM("Holding Position: x = " << currentVehiclePose_.pose.position.x << ", y = " << currentVehiclePose_.pose.position.y << ", z = " << currentVehiclePose_.pose.position.z);
            ...
            {
                if(verbose_){ ROS_WARN_STREAM("* amaze_mission_sequencer::request::HOLD - failed! Not in MISSION!"); }
                wrongInput = true;
            }
            break;
        case amaze_mission_sequencer::request::RESUME:
            ...
            {
                if(verbose_){ ROS_WARN_STREAM("* amaze_mission_sequencer::request::RESUME - failed! Not in HOLD!"); }
                wrongInput = true;
            }
            break;

        case amaze_mission_sequencer::request::ABORT:
            if(verbose_){ ROS_INFO_STREAM("* amaze_mission_sequencer::request::ABORT..."); }
        ...
        case amaze_mission_sequencer::request::LAND:
            if(verbose_){ ROS_INFO_STREAM("* amaze_mission_sequencer::request::LAND..."); }
            if (rosServiceLand_.call(landCmd_))
            {
                if (landCmd_.response.success)
                {
...
geometry_msgs::PoseStamped AmazeMissionSequencer::waypointToPoseStamped(const ParseWaypoint::Waypoint& waypoint)
{
    ...
    if (relWaypoints_)
    {
        tf2::Quaternion startingQuaternion(startingVehiclePose_.pose.orientation.x, startingVehiclePose_.pose.orientation.y,startingVehiclePose_.pose.orientation.z, startingVehiclePose_.pose.orientation.w);
        ...
        pose.pose.position.x = (waypoint.x*cos(startingYaw) - waypoint.y*sin(startingYaw)) + startingVehiclePose_.pose.position.x;
        pose.pose.position.y = (waypoint.x*sin(startingYaw) + waypoint.y*cos(startingYaw)) + startingVehiclePose_.pose.position.y;
...
void AmazeMissionSequencer::logic(void)
{
    ...
    {
        case IDLE:
            if(verbose_){ ROS_INFO_STREAM_THROTTLE(dbg_throttle_rate_, "* currentFollowerState__::IDLE"); }
            return;

        case PREARM:
            if(verbose_){ ROS_INFO_STREAM_THROTTLE(dbg_throttle_rate_, "* currentFollowerState__::PREARM"); }
            return;

        case ARM:
            if(verbose_){ ROS_INFO_STREAM_THROTTLE(dbg_throttle_rate_, "* currentFollowerState__::ARM"); }
        ...
        case MISSION:
            if (waypointList_.size() > 0)
            {
                ...
                differenceYaw = std::abs(2.0*double(tf2::Quaternion(currentVehiclePose_.pose.orientation.x, currentVehiclePose_.pose.orientation.y,currentVehiclePose_.pose.orientation.z, currentVehiclePose_.pose.orientation.w).angle(tf2::Quaternion(currentWaypoint.pose.orientation.x, currentWaypoint.pose.orientation.y, currentWaypoint.pose.orientation.z, currentWaypoint.pose.orientation.w))));
                ...
                {
                    ROS_INFO_STREAM("Reached Waypoint: x = " << waypointList_[0].x << ", y = " << waypointList_[0].y << ", z = " << waypointList_[0].z << ", yaw = " << waypointList_[0].yaw);
            ...
            {
                if(verbose_){ ROS_INFO_STREAM_THROTTLE(dbg_throttle_rate_, "* No more waypoints to follow..."); }
            ...
            if(verbose_){ ROS_INFO_STREAM_THROTTLE(dbg_throttle_rate_, "* currentFollowerState__::MISSION; waypoints left: " << waypointList_.size()); }
            break;
        ...
        case LAND:
            if (currentExtendedVehicleState_.landed_state == currentExtendedVehicleState_.LANDED_STATE_ON_GROUND)
            {
                ...
                if (!currentVehicleState_.armed)
                {
                    if(verbose_){ ROS_INFO_STREAM_THROTTLE(dbg_throttle_rate_, "* currentFollowerState__::LAND->LANDED --> set state to IDLE"); }
                    currentFollowerState_ = IDLE;
                }
                else
                {
                    if(verbose_){ ROS_INFO_STREAM_THROTTLE(dbg_throttle_rate_, "* currentFollowerState__::LAND->LANDED --> Vehicle still ARMED"); }
                }
            }
            break;
